  ...
  server: {
    handlers: {
      POST: async () => {
        ...
        const [{ data: tasks }, { data: overrides }, { data: courses }, { data: sessions }] =
          await Promise.all([
            supabase
              .from("tasks")
        ...
        for (const settings of activeUsers) {
          ...
          const recipientEmail =
            settings.reminder_email && settings.reminder_email_verified
          ...
          const displayName =
            info?.displayName || displayNames.get(settings.user_id) || recipientEmail.split("@")[0];
              ...
              for (const offsetMin of offsets) {
                ...
                if (noTime) {
                  ...
                  scheduledTime = new Date(
                    `${targetLocalDateStr}T${String(settings.reminder_fallback_hour).padStart(2, "0")}:00:00`,
                  );
                } else {
          ...
          if (settings.daily_summary_enabled && localHour(now, tz) === 7) {
            ...
            if (!dupErr) {
              ...
              const todaySessions = (sessions ?? [])
                .filter(
                  (s) =>
                    s.user_id === settings.user_id &&
                    s.planned_start &&
                    localDateStr(new Date(s.planned_start), tz) === dateStr,
                )
                .map((s) => ({
                  title: (s.course_id && courseMap.get(s.course_id)) || "Studiepass",
                  startLabel: new Intl.DateTimeFormat("sv-SE", {
                    timeZone: tz,
                    hour: "2-digit",
                    minute: "2-digit",
                  }).format(new Date(s.planned_start!)),
                }));
          ...
          if (
            ...
            if (!dupErr) {
              ...
              const totalSec = (entries ?? []).reduce(
                (a, e) => a + ((e as { duration_seconds: number }).duration_seconds ?? 0),
                0,
              );
