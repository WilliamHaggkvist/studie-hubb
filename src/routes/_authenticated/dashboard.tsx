...
function periodMatches(coursePeriod: string | null, activePeriod: TermRow["term"] | null): boolean {
  ...
  if (
    activePeriod === "host" &&
    (coursePeriod.startsWith("host") || coursePeriod === "period-1" || coursePeriod === "period-2")
  )
    return true;
  if (
    activePeriod === "var" &&
    (coursePeriod.startsWith("var") || coursePeriod === "period-3" || coursePeriod === "period-4")
  )
    return true;
...
function Dashboard() {
  ...
  const completedOrPendingTasksToday = todayTasks.filter(
    (t) => t.status === "done" || t.pending_review,
  ).length;
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 lg:px-8">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Button
          ...
          className="gradient-sunset text-white hover:opacity-90 rounded-xl sm:self-end gap-1.5 self-start"
        >
                    ...
                    {periodCourses.map((c) => {
                      ...
                      return (
                        <Link
                          ...
                          className="group rounded-xl glass border-white/5 p-4 transition-colors hover:border-primary/40 hover:bg-white/5"
                        >
                          ...
                          {goal > 0 ? (
                            <>
                              <div className="mt-3 flex items-center justify-between text-xs">
                                ...
                                <span className="tabular-nums">
                                  {hoursThisWeek.toFixed(2)} / {goal} h
                                </span>
                              </div>
                            ...
                            <div className="mt-3 text-xs text-muted-foreground">
                              Inget veckomål · {hoursThisWeek.toFixed(2)} h denna vecka
                            </div>
                          )}
                        </Link>
                      );
          ...
          <CardContent className="space-y-4">
              ...
              <div className="relative flex items-center justify-center shrink-0">
                <svg className="h-20 w-20" viewBox="0 0 100 100">
                  <circle
                    ...
                    style={{
                      fillOpacity:
                        totalTasksToday > 0 ? (taskPct === 100 ? 0.2 : (taskPct / 100) * 0.08) : 0,
                    }}
                  />
                  <circle
                    cx="50"
              ...
              <div className="min-w-0 flex-1 space-y-0.5">
                ...
                <p className="text-[11px] leading-relaxed text-muted-foreground">
                  {totalTasksToday === 0 ? (
                    ...
                    <>
                      Du har gjort klart{" "}
                      <span className="font-bold text-white">{completedOrPendingTasksToday}</span>{" "}
                      av <span className="font-bold text-white">{totalTasksToday}</span>{" "}
                      {totalTasksToday === 1 ? "uppgift" : "uppgifter"}.
                    </>
                  )}
                </p>
            ...
            <div>
              ...
              {todaysSessions.length === 0 && (
                <div className="text-sm text-muted-foreground">Inga planerade pass.</div>
              )}
              ...
              {todayTasks.map((t) => {
                ...
                const statusConfig: Record<TaskStatus, { label: string; style: string }> = {
                  todo: {
                    label: "Ej startad",
                    style:
                      "border-sunset-rose/30 text-sunset-rose bg-sunset-rose/10 hover:bg-sunset-rose/20",
                  },
                  doing: {
                    label: "Pågår",
                    style:
                      "border-sunset-amber/30 text-sunset-amber bg-sunset-amber/10 hover:bg-sunset-amber/20",
                  },
                  done: {
                    label: "Klar",
                    style:
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      style={{ background: c?.color ?? "var(--sunset-violet)" }}
                    />
                    <span className="tabular-nums text-xs text-muted-foreground shrink-0">
                      {t.due_at ? format(parseISO(t.due_at), "HH:mm") : "--:--"}
                    </span>
                    <span
                      className={`rounded-full px-1.5 py-0.5 text-[9px] uppercase tracking-wider shrink-0 ${TYPE_COLORS[t.task_type as TaskType]}`}
                    >
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-1.5 text-center text-xs">
                <div className="rounded-lg bg-white/5 p-1.5">
                  <div className="text-muted-foreground text-[9px] uppercase font-bold">
                    Ej startad
                  </div>
                  <div className="text-sm font-semibold mt-0.5 tabular-nums text-sunset-rose">
                    {weekTodoCount}
                  </div>
                </div>
                <div className="rounded-lg bg-white/5 p-1.5">
                  <div className="text-muted-foreground text-[9px] uppercase font-bold">Pågår</div>
                  <div className="text-sm font-semibold mt-0.5 tabular-nums text-sunset-amber">
                    {weekDoingCount}
                  </div>
                </div>
                <div className="rounded-lg bg-white/5 p-1.5">
                  <div className="text-muted-foreground text-[9px] uppercase font-bold">Klar</div>
                  <div className="text-sm font-semibold mt-0.5 tabular-nums text-emerald-400">
                    {weekDoneCount}
                  </div>
                </div>
              </div>
