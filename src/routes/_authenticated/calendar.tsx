    ...
    <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Deadlines, händelser och studiepass — samlat och skrivskyddat. Skapa uppgifter/pass från respektive vy.</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex rounded-md border border-border/60 p-0.5 text-xs">
            <button onClick={() => setView("month")} className={cn("rounded px-2 py-1", view === "month" && "bg-surface-2 font-medium")}>Månad</button>
            <button onClick={() => setView("week")} className={cn("rounded px-2 py-1", view === "week" && "bg-surface-2 font-medium")}>Vecka</button>
          </div>
          ...
          <div className="min-w-[10rem] text-center font-display text-lg">
            {view === "month" ? format(cursor, "MMMM yyyy", { locale: sv }) : `Vecka ${format(cursor, "w")} · ${format(cursor, "MMM yyyy", { locale: sv })}`}
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {items.length > 0 && <span className="text-[9px] text-muted-foreground">{items.length}</span>}
                  </div>
                  <div className="space-y-0.5">
                    {items.slice(0, 3).map((it) => {
                      if (it.kind === "session") {
                        ...
                        return (
                          <div key={"s"+it.ref.id} className="truncate rounded px-1 py-0.5 text-[10px]" style={{ background: `${c?.color ?? "#8B5CF6"}22`, borderLeft: `2px solid ${c?.color ?? "#8B5CF6"}` }}>
                      ...
                      if (it.kind === "event") {
                        const c = courses.find((c) => c.id === it.ref.course_id);
                        return (
                          <div key={"e"+it.ref.id} className="truncate rounded px-1 py-0.5 text-[10px]" style={{ background: `${c?.color ?? "#8B5CF6"}33`, color: c?.color ?? "var(--sunset-violet)" }}>
                            {format(parseISO(it.ref.starts_at), "HH:mm")} {it.ref.title}
                          </div>
                        );
                      }
                      return (
                        <div key={"t"+it.ref.id} className="truncate rounded border border-dashed px-1 py-0.5 text-[10px] text-sunset-amber" style={{ borderColor: "var(--sunset-amber)" }}>
                          ⚑ {it.ref.title}
                        </div>
                      );
                    })}
                    {items.length > 3 && <div className="text-[9px] text-muted-foreground">+{items.length - 3} till</div>}
                  </div>
                </button>
              );
            })}
            ...
            {selectedSessions.map((s) => {
              ...
              return (
                <div key={s.id} className="rounded-lg border border-border/60 bg-surface p-3">
                  <div className="flex items-center gap-2 text-xs uppercase tracking-wider" style={{ color: c?.color ?? "var(--sunset-violet)" }}>
                  ...
                  <div className="mt-0.5 flex items-center gap-1 text-xs text-muted-foreground">
                    ...
                    {format(parseISO(s.planned_start), "HH:mm")}–{format(parseISO(s.planned_end), "HH:mm")}
                  </div>
            ...
            {selectedEvents.map((e) => {
              ...
              return (
                <div key={e.id} className="rounded-lg border border-border/60 bg-surface p-3">
                  ...
                  {e.source === "google" && <span className="ml-2 rounded bg-surface-2 px-1.5 py-0.5 text-[9px] uppercase text-muted-foreground">Google</span>}
                </div>
              );
            })}
            {selectedTasks.map((t) => {
              ...
              return (
                <div key={t.id} className="rounded-lg border border-dashed border-sunset-amber/60 bg-surface p-3 text-sm">
                  ...
                  <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
                    <Clock className="h-3 w-3" />{format(parseISO(t.due_at!), "HH:mm")}
                    {c && <span className="inline-flex items-center gap-1" style={{ color: c.color }}><span className="inline-block h-1.5 w-1.5 rounded-full" style={{ background: c.color }} />{c.name}</span>}
                  </div>
                </div>
              );
            })}
