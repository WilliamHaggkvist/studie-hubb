      ...
      <CardContent className="space-y-3">
        <div className="space-y-1.5">
          ...
          {terms.map((t) => {
            ...
            const arskurs = getArskursFromAcademicYear(ay);
            return (
              <div
                key={t.id}
                className="flex flex-wrap items-center gap-2 rounded-xl border border-border/60 bg-background/40 px-3 py-2 text-sm"
              >
                <span className="font-medium">
                  {termName(t.term)} {t.year}
                </span>
                {arskurs && (
                  <span className="rounded-lg bg-sky-500/10 border border-sky-500/20 px-2 py-0.5 font-display text-[11px] font-bold text-sky-400">
                    Årskurs {arskurs}
                  </span>
                )}
                ...
                <Button
                  size="icon"
                  variant="ghost"
                  className="ml-auto sm:ml-0 rounded-xl text-destructive"
