...
interface SessionItem { title: string; startLabel: string }
interface Props {
  displayName?: string
  dateLabel?: string
  tasks?: TaskItem[]
    ...
    <Body style={main}>
      <Container style={container}>
        ...
        <Section style={card}>
          ...
          <Button href={appUrl} style={button}>Öppna StudieHubb</Button>
        </Section>
      </Container>
    </Body>
  </Html>
...
export const template = {
  component: Email,
  subject: (d: Record<string, unknown>) => `Din dag i StudieHubb — ${(d.dateLabel as string) ?? ''}`,
...
const h1 = { fontSize: '22px', lineHeight: '28px', fontWeight: 700, color: '#1a1a1a', margin: '0 0 16px' }
